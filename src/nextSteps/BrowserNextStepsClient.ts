...
export class BrowserNextStepsClient implements NextStepsClient {
  ...
  async analyzeReference(file: File): Promise<NextStepsAnalysis> {
    ...
    console.info('[Sidekick] Browser Gemini Next Steps request started.', {
      model: this.model,
      fileName: file.name,
      fileSize: file.size
    });
    ...
    if (!response.ok) {
      console.error('[Sidekick] Browser Gemini Next Steps request failed.', {
        model: this.model,
    ...
    if (!parsed) {
      console.error('[Sidekick] Browser Gemini returned non-JSON Next Steps payload.', {
        model: this.model,
