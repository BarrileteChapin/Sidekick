...
export class BrowserGeminiAdapter implements GeminiAdapter {
  ...
  async createPlan(input: GeminiPlanInput): Promise<ChatGenerationPlan> {
    ...
    console.info('[Sidekick] Browser Gemini chat planning request started.', {
      model: this.model
    });
    ...
    if (!response.ok) {
      console.error('[Sidekick] Browser Gemini chat planning request failed.', {
        model: this.model,
    ...
    if (!parsed) {
      console.error('[Sidekick] Browser Gemini returned non-JSON chat plan payload.', {
        model: this.model,
        payloadPreview: typeof text === 'string' ? text.slice(0, 120) : String(text)
      });
